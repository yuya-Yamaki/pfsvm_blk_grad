...
int main(int argc, char **argv)
{
    ...
    size_t elements;
    double th_list[MAX_CLASS/2], fvector[NUM_FEATURES], sig_gain = 0.125;/*特徴ベクトル12次元，シグモイド関数の利得係数0.125*/
    const char *error_msg;
    static double svm_c = 2.0, svm_gamma = 0.25 / NUM_FEATURES;
    ...
    int count;//add
    int rotnum, blkcorner=0;  //add
    int blkcorner_x, blkcorner_y;
...
return ((double)dif / CLOCKS_PER_SEC);*/
    ...
    set_inthresholds(oimg_list, dimg_list, num_img, num_class, th_list);
